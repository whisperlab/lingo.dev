import Z from "zod";
import { LocaleCode, localeCodeSchema } from "@whisperlab/lingo.dev_spec";
import { createId } from "@paralleldrive/cuid2";
import { generateText, LanguageModelV1 } from "ai";

const engineParamsSchema = Z.object({
  model: Z.any() satisfies Z.ZodType<LanguageModelV1>,
...
const localizationParamsSchema = Z.object({
  sourceLocale: localeCodeSchema, // Z.union([localeCodeSchema, Z.null()]),
...
/**
 * BasicEngine class for interacting with the LingoDotDev API
...
export class BasicEngine {
  ...
  /**
   * Create a new BasicEngine instance
      ...
      messages: [
        {
          ...
          content: JSON.stringify({
            role: "system",
            content: this.config.systemPrompt.replaceAll("{source}", sourceLocale).replaceAll("{target}", targetLocale),
          }),
        },
        {
          role: "user",
  ...
  async recognizeLocale(text: string): Promise<LocaleCode> {
      ...
      messages: [
        {
          ...
          content: JSON.stringify({
            role: "system",
            content: "Detect language (locale) accurately",
          }),
        },
        {
          role: "user",
    ...
    if (!result.locale) {
      throw new Error(`Error recognizing locale`);
    }
